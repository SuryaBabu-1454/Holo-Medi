...
const MediFinder = () => {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      const processedBooks = booksData.map(book => ({
        ...book,
        thumbnail: book.thumbnail || 'https://via.placeholder.com/150x200?text=No+Cover',
      ...
      const newHistory = [
        searchQuery,
        ...history.filter(item => item.toLowerCase() !== searchQuery.toLowerCase())
      ].slice(0, 5);
  ...
  const shuffledResults = [...books.map(b => ({ type: 'book', data: b })), 
                       ...
                       const renderContent = () => {
                        ...
                        if (activeTab === 'books' && !hasBooks) {
                          return (
                            <div className="col-span-full text-center py-10">
                              ...
                              <p className="text-gray-500">Try a different search term or check the videos tab</p>
                            </div>
                          );
                        }
                        ...
                        if (activeTab === 'videos' && !hasVideos) {
                          return (
                            <div className="col-span-full text-center py-10">
                              ...
                              <p className="text-gray-500">Try a different search term or check the books tab</p>
                            </div>
                          );
                        }
                        ...
                        return shuffledResults.map((item, index) => {
                          if (item.type === 'book' && (activeTab === 'all' || activeTab === 'books')) {
                            return (
                              <motion.div 
                              ...
                              >
                                <div className="border rounded-lg shadow hover:shadow-lg bg-green-50 h-48 overflow-hidden relative group">
                                  <MdOutlineLibraryBooks className="absolute top-2 right-2 bg-black p-1 rounded-full text-green-500 z-10" size={24} />
                                  <img
                                  ...
                                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-2">
                                    <h3 className="text-white font-medium text-sm line-clamp-1">{item.data.title}</h3>
                                    <p className="text-white/80 text-xs">
                                      {item.data.authors.join(', ')}
                                      {item.data.publishedDate && ` • ${item.data.publishedDate.substring(0, 4)}`}
                                    </p>
                                  </div>
                                </div>
                              </motion.div>
                          ...
                          if (item.type === 'video' && (activeTab === 'all' || activeTab === 'videos')) {
                            return (
                              <motion.div 
                              ...
                              >
                                <a href={item.data.url} target="_blank" rel="noopener noreferrer">
                                  <div className="border rounded-lg shadow hover:shadow-lg bg-blue-50 h-48 overflow-hidden relative group">
                                    <FaRegPlayCircle className="absolute top-2 right-2 bg-black p-1 rounded-full text-red-500 z-10" size={24} />
                                    <img
                                      src={item.data.thumbnail}
                                      alt={item.data.title}
                                      className="w-full h-full object-cover group-hover:opacity-90 transition-opacity"
                                      loading="lazy"
                                      onError={(e) => e.target.src = 'https://via.placeholder.com/300x200?text=No+Thumbnail'}
                                    />
                                    <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-2">
                                      <h3 className="text-white font-medium text-sm line-clamp-1">{item.data.title}</h3>
                                      <p className="text-white/80 text-xs">
    ...
    <div className="p-4 max-w-6xl mx-auto min-h-screen">
      ...
      {!hasSearched && (
        <div className="text-center mb-6">
          <p className="text-gray-600 mx-10 text-lg">
            Discover medical books and educational videos. Search for topics like anatomy, 
            pharmacology, or any medical subject to find relevant resources.
          </p>
        </div>
      )}
      ...
      {history.length > 0 && !hasSearched && (
        <div className="mb-6 bg-gray-50 p-4 rounded-lg max-w-4xl mx-auto">
          <h3 className="font-medium text-gray-700 mb-2 text-center">Recent Searches:</h3>
